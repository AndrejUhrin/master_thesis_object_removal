...
base_path = Path("../inpaint_blur_results/canny_inpaint_blur_results")
...
canny_ann = base_path / "canny_inpaint_blur"

for d in (base_path,pre_raw_ann, post_ann_out,
          pre_json_out, post_json_out,canny_ann
          ):
...
for idx, img_path in enumerate(all_imgs, start=1):
    ...
    if not post_enl:
        print("no boxes detected, not running SAM.\n")
        continue
    ...
    print("Saving Canny annotation image")
    ...
    for item in classified_contours:
        ...
        label_txt = (f"{action}, {edge_ratio:.3f}"if edge_ratio == edge_ratio else f"{action}")
    ...
    annotated_image.save(canny_ann / f"{stem}.jpg", quality=jpeg_q)
    print("Saved Canny annotation image in new style.\n")
